...
class SimpleViewController: TableCollectionViewController<SimpleSection> {
    ...
    private lazy var toolBar: UIToolbar = {
        let toolBar = UIToolbar(frame: CGRect(x: 0, y: 64, width: self.view.frame.width, height: 44))
    ...
    override func viewDidLoad() {
        ...
        collectionView.contentInset = UIEdgeInsets(top: toolBar.frame.height + 10, left: 0, bottom: 0, right: 0)
        ...
        toolBar.items = [
            UIBarButtonItem(barButtonSystemItem: .refresh, target: self, action: #selector(randomAdd)),
